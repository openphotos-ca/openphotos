...
impl FaceNormalizer {
    ...
    pub fn normalize_face(&self, image: &DynamicImage, detection: &FaceDetection) -> Result<RgbImage> {
        ...
        debug!("Normalized face to {}x{}", self.target_size, self.target_size);
        Ok(aligned_face)
    }
    
    /// Simple crop and resize approach to match Python InsightFace behavior
    fn simple_crop_resize(&self, image: &DynamicImage, bbox: &crate::types::BoundingBox) -> Result<RgbImage> {
        ...
        debug!("Image: {}x{}, BBox: ({:.3}, {:.3}, {:.3}, {:.3})", 
               img_width, img_height, bbox.x1, bbox.y1, bbox.x2, bbox.y2);
        debug!("Cropping face: ({}, {}) -> ({}, {}), size: {}x{}", x1, y1, x2, y2, width, height);
        
        // Ensure minimum dimensions
        if width < 10 || height < 10 {
            return Err(anyhow::anyhow!("Face region too small: {}x{}", width, height));
        }
        ...
        let resized = cropped.resize_exact(self.target_size, self.target_size, image::imageops::FilterType::Lanczos3);
    ...
    fn estimate_norm(&self, src_points: &[(f32, f32)], dst_points: &[(f32, f32)]) -> Result<Matrix2x3<f32>> {
        ...
        let params = svd.solve(&dst_vec, 1e-6)
