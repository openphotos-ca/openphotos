...
impl FaceDetector {
    ...
    fn detect_with_retinaface(
        ...
        info!(
            "[FACE] detect_with_retinaface start dims={}x{}",
        ...
        debug!(
            "Detection scale: {:.3} (resized to {}x{} from {}x{})",
        ...
        // If no faces detected with ONNX, do NOT fall back to placeholder in production.
        // Enable placeholder only when FACE_ALLOW_PLACEHOLDER=1 for testing.
        if detections.is_empty() {
            let allow_placeholder = std::env::var("FACE_ALLOW_PLACEHOLDER")
                ...
                .unwrap_or(false);
            if allow_placeholder {
                warn!("No faces detected with RetinaFace, using placeholder detection (FACE_ALLOW_PLACEHOLDER=1)");
                detections = self.generate_placeholder_detection(image)?;
            } else {
                debug!("No faces detected with RetinaFace (placeholder disabled)");
            }
        }
    ...
    fn preprocess_image(&self, image: &image::RgbImage) -> Result<Value> {
        ...
        debug!(
            "Aspect ratio preprocessing: {}x{} -> {}x{} -> padded to {}x{}",
        ...
        for y in 0..new_height {
            for x in 0..new_width {
                ...
                array[[0, 2, y as usize, x as usize]] = (pixel[0] as f32 - 127.5) / 128.0;
                // R
            }
        }
    ...
    fn postprocess_retinaface_outputs(
        ...
        for (stride, cls_key, bbox_key, ldm_key) in stride_configs {
                ...
                (cls_output, bbox_output, ldm_output)
            {
                ...
                if cls_data.len() != expected_total {
                    warn!(
                        "Unexpected tensor size for stride {}: got {}, expected {}",
                        stride,
                        cls_data.len(),
                        expected_total
                    );
                ...
                debug!(
                    "Stride {} raw logits range: [{:.3}, {:.3}]",
                    stride, min_raw, max_raw
                );
                ...
                for (idx, &score) in cls_data.iter().enumerate() {
                    ...
                    if prob > self.min_score && idx < anchors.len() {
                        ...
                        if bbox_start + 4 <= bbox_data.len() && ldm_start + 10 <= ldm_data.len() {
                            ...
                            let scaled_deltas: Vec<f32> =
                                bbox_deltas.iter().map(|&d| d * stride as f32).collect();

                            // Decode bounding box
                            let decoded_box = self.decode_box(anchor, &scaled_deltas);

                            // Scale landmark deltas by stride (as per InsightFace implementation)
                            let scaled_ldm_deltas: Vec<f32> =
                                ldm_deltas.iter().map(|&d| d * stride as f32).collect();

                            // Decode landmarks
                            let decoded_landmarks =
                                self.decode_landmarks(anchor, &scaled_ldm_deltas);
                ...
                debug!(
                    "Stride {} stats: max_conf={:.3}, above_0.5={}, valid={}",
                    stride,
                    max_confidence,
        ...
        for i in 0..all_boxes.len() {
            ...
            if bbox[0] >= 0.0
                && bbox[1] >= 0.0
                && bbox[2] <= orig_width as f32 * 1.1
                && bbox[3] <= orig_height as f32 * 1.1
                && bbox[2] > bbox[0]
                && bbox[3] > bbox[1]
            {
                ...
                debug!(
                    "Filtered out invalid box: [{:.1}, {:.1}, {:.1}, {:.1}] (image: {}x{})",
        ...
        debug!(
            "Valid detections after bounds filtering: {}/{}",
            valid_boxes.len(),
            all_boxes.len()
        );
        ...
        for &idx in sorted_indices.iter() {
            if idx < valid_boxes.len() {
                ...
                debug!(
                    "Added detection: bbox=[{:.1},{:.1},{:.1},{:.1}], conf={:.3}",
    ...
    fn create_detection_from_score(
        ...
        let landmarks = FacialLandmarks::new(vec![
            (
                x1 as f32 + face_size as f32 * 0.35,
                y1 as f32 + face_size as f32 * 0.35,
            ), // left eye
            (
                x1 as f32 + face_size as f32 * 0.65,
                y1 as f32 + face_size as f32 * 0.35,
            ), // right eye
            (
                x1 as f32 + face_size as f32 * 0.5,
                y1 as f32 + face_size as f32 * 0.5,
            ), // nose
            (
                x1 as f32 + face_size as f32 * 0.35,
                y1 as f32 + face_size as f32 * 0.75,
            ), // left mouth
            (
                x1 as f32 + face_size as f32 * 0.65,
                y1 as f32 + face_size as f32 * 0.75,
    ...
    fn generate_anchors(&self, stride: i32, feat_h: usize, feat_w: usize) -> Vec<[f32; 4]> {
        ...
        debug!(
            "Generating anchors: {}x{} feature map, 2 anchors per location",
            feat_w, feat_h
        );
