...
impl FaceRecognizer {
    ...
    pub fn generate_embedding(&self, normalized_face: &RgbImage) -> Result<Vec<f32>> {
        if let Some(session_arc) = &self.session {
            self.generate_arcface_embedding(normalized_face, session_arc)
        } else {
    ...
    fn generate_arcface_embedding(
        ...
        debug!(
            "Generated ArcFace embedding with dimension {}",
            embedding.len()
        );
        Ok(embedding)
    }
    ...
    fn preprocess_face(&self, face: &RgbImage) -> Result<Value> {
        ...
        let mut array =
            Array4::<f32>::zeros((1, 3, self.input_size.1 as usize, self.input_size.0 as usize));
        ...
        for y in 0..self.input_size.1 {
            for x in 0..self.input_size.0 {
                ...
                array[[0, 2, y as usize, x as usize]] = (pixel[0] as f32 - 127.5) / 127.5;
                // R
            }
        }
    ...
    fn generate_placeholder_embedding(&self, face: &RgbImage) -> Result<Vec<f32>> {
        ...
        debug!(
            "Generated placeholder embedding with dimension {}",
            embedding.len()
        );
        Ok(embedding)
    }
