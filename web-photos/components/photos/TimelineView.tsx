...
function StarRatingOverlay({ assetId, initialRating, interactive }: { assetId: string; initialRating?: number | null; interactive?: boolean }) {
  ...
  return (
    <div className={`absolute bottom-0 left-0 right-0 p-1 ${canInteract ? 'bg-black/40' : 'bg-transparent'} z-10 ${canInteract ? 'pointer-events-auto cursor-pointer' : 'pointer-events-none cursor-default'}`} onDoubleClick={(e)=>{ if (!canInteract) return; e.stopPropagation(); setEditing(true); }}>
