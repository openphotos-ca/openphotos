      ...
      <div className="absolute inset-0 grid place-items-center p-4">
        <div className="w-full max-w-sm bg-background text-foreground border border-border rounded-lg shadow-2xl">
          <div className="px-4 py-3 border-b border-border font-medium flex items-center justify-between gap-3">
            <div>{tx('Unlock')}</div>
            <button className="px-2 py-1 text-xs border border-border rounded hover:bg-muted" onClick={onClose}>{tx('Close')}</button>
          </div>
          <div className="p-4 space-y-3">
            {!e2ee.envelope ? (
              <div className="text-sm text-muted-foreground">{tx('No PIN set. Go to Settings → Security to create one.')}</div>
            ) : (
              <>
                <div>
                  <div className="mb-2 text-sm">{tx('Enter your 8-character PIN')}</div>
                  <PinInput value={pin} onChange={(v)=>{ setPin(v); setError(''); }} autoFocus ariaLabel={tx('PIN')} />
                </div>
                ...
                <div className="pt-1 flex items-center justify-end">
                  <button
                    onClick={async () => {
                      ...
                      try {
                        ...
                        try { toast({ title: tx('Unlocked'), description: tx('E2EE session is now unlocked'), variant: 'success' }); } catch {}
                        onClose();
                      } catch (e:any) { setError(e?.message||tx('Unlock failed')); }
                  ...
                  >{busy ? tx('Unlocking…') : tx('Unlock')}</button>
                </div>
              </>
            )}
          </div>
